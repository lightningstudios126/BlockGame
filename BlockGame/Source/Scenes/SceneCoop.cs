...
#pragma warning restore CS8618 // Non-nullable field is uninitialized. Consider declaring as nullable.
		...
		public override void Initialize() {
			...
			table.Add(new PlayfieldWidget(playfield));
		}
		...
		private void InitializeGameElements() {
			...
			playfield = CreateEntity("playfield").AddComponent(new Playfield(15));
			playfield.Transform.Position = new Vector2(Screen.Width / 2 - playfield.Width / 2, 620);

			{
				var outline = Color.OrangeRed;
				var leftEdge = playfield.Transform.Position.X;
				...
				controller1.AddComponent(new PlayerController(playfield) { nextQueue = localNextQueue, holdQueue = localHoldQueue, outlineTint = outline });
			}

			{
				var outline = Color.Turquoise;
				var rightEdge = playfield.Transform.Position.X + playfield.Width;
				...
				controller2.AddComponent(new PlayerController(playfield) { nextQueue = localNextQueue, holdQueue = localHoldQueue, outlineTint = outline, spawnLocation = new Point(10, 20) });
			}
		}

		public override void OnStart() {
			base.OnStart();
			Console.WriteLine(string.Join(", ", playfield.FullRows));
		}
